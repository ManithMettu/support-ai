...
const rawDocs = JSON.parse(fs.readFileSync(new URL("./docs.json", import.meta.url), "utf-8"));
const docs: DocumentChunk[] = initializeEmbeddings(rawDocs);
...
export async function registerRoutes(
  ...
  app.post(api.chat.create.path, async (req, res) => {
    try {
      ...
      const relevantDocs = findRelevantDocs(input.message, docs, 3);
      const relevantDocsText = relevantDocs
        .map(doc => `Title: ${doc.title}\nContent: ${doc.content}`)
        .join("\n\n");

      // Construct Prompt with only relevant docs
      const systemPrompt = `You are a helpful AI support assistant.
      
Use the following relevant product documentation to answer the user's questions:

${relevantDocsText}

Strict Rules:
- Generate responses ONLY using the documentation provided above.
- If the user asks something outside these docs, you MUST respond exactly with: "Sorry, I don't have information about that."
